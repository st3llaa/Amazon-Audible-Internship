...
public class ClearListHandler implements RequestHandler {
    ...
    public boolean canHandle(HandlerInput input) {
        return input.matches(intentName("ClearListIntent"));
    }
    ...
    public Optional<Response> handle(HandlerInput input) {
        log(input, "Starting clear list request");
        //logSlots(input);
        Map<String, Slot> slots = getSlots(input);
        String listName = slots.get("ListNameSlot").getValue();
        AlexaSessionDynamoDBHandler.clear(input, listName);
        String speechTextWithBook = "<speak> %s list successfully deleted </speak>";
        String speechText = String.format(speechTextWithBook, listName);
