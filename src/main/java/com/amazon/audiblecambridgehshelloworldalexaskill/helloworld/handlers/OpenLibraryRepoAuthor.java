...
public class OpenLibraryRepoAuthor {

    public Book searchBook(String authorName) {
        // calls buildHttpGetRequest method with input search key to build http request.
        HttpURLConnection httpURLConnection = buildHttpGetRequest(authorName);;
    ...
    private HttpURLConnection buildHttpGetRequest(String authorName) {
        try {
            String encodedSearchKey = URLEncoder.encode(authorName,"UTF-8");
            String urlString = "http://openlibrary.org/search.json?author="+encodedSearchKey+"&fields=title,author_name,key&limit=1"; // TODO: build request url with query, see https://openlibrary.org/dev/docs/api/search for url examples
            System.out.println("Open library URL: " + urlString);
